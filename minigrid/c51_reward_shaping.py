...
if __name__ == "__main__":
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env(
                args.env_id, args.seed + i, args.n_keys, i, args.capture_video, run_name
            )
    ...
    for global_step in progress_bar:
        ...
        suggested_actions = q_network._apply_rules_batch(
            q_network.get_observables(torch.as_tensor(obs[:, 4:], device=device))
        )

        # Keep only valid integer suggestions (drop None)
        valid_suggestions = [
            int(a)
            for a in suggested_actions[0]
            if a is not None and isinstance(a, (int, np.integer))
        ]
