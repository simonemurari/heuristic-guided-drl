...
class OfficeWorldEnv(GridWorldEnv):
    ...
    def step(self, action):
        ...
        reward, is_done = self._get_reward(is_action_optimal), self.is_terminal()
    ...
    def get_observations(self):
        ...
        for location in self.locations:
            if location == self.agent:
                observations[location] = self.locations[location]

        if self.agent in self.coffee:
            observations["coffee"] = OfficeWorldObject.COFFEE

        if self.agent == self.mail:
            observations["mail"] = OfficeWorldObject.MAIL
    ...
    def _hitting_plant(self, target_pos):
        return target_pos in self.locations and self.locations[target_pos] == OfficeWorldObject.PLANT
...
class OfficeWorldDeliverCoffeeEnv(OfficeWorldEnv):
    ...
    def _get_suggested_actions(self, agent_pos, target_positions):
        """Returns a set of suggested actions (up, down, left, right) to guide the agent towards the target position."""
        
        suggested_actions = []
        for target_pos in target_positions:    
            # Determine direction towards the target in terms of x (left-right) and y (up-down)
            x_diff = target_pos[0] - agent_pos[0]
            y_diff = target_pos[1] - agent_pos[1]
            
            # Check up, down, left, right relative to the target
            if x_diff < 0 and self._is_valid_movement(agent_pos, (agent_pos[0]-1, agent_pos[1])) \
                and not self._hitting_plant((agent_pos[0]-1, agent_pos[1])): 
                suggested_actions.append(GridWorldActions.LEFT)
            if x_diff > 0 and self._is_valid_movement(agent_pos, (agent_pos[0]+1, agent_pos[1])) \
                and not self._hitting_plant((agent_pos[0]+1, agent_pos[1])): 
                suggested_actions.append(GridWorldActions.RIGHT)
            if y_diff > 0 and self._is_valid_movement(agent_pos, (agent_pos[0], agent_pos[1]+1)) \
                and not self._hitting_plant((agent_pos[0], agent_pos[1]+1)):
                suggested_actions.append(GridWorldActions.UP)
            if y_diff < 0 and self._is_valid_movement(agent_pos, (agent_pos[0], agent_pos[1]-1)) \
                and not self._hitting_plant((agent_pos[0], agent_pos[1]-1)):
                suggested_actions.append(GridWorldActions.DOWN)

        return suggested_actions

    def guide_agent(self):
        if self.has_coffee:
            # The agent has the coffee and needs to reach the office
            locs = set()
            for loc, elem in self.locations.items():
                if elem == OfficeWorldObject.OFFICE:
                    locs.add(loc)
            return self._get_suggested_actions(self.agent, locs)
        else:
            # The agent still needs to reach the coffee
            return self._get_suggested_actions(self.agent, self.coffee)
...
class OfficeWorldDeliverMailEnv(OfficeWorldEnv):
    ...
    def guide_agent(self):
        if self.has_mail:
            ...
            # The agent still needs to reach the coffee
            return self._get_suggested_actions(self.agent, [self.mail])
...
class OfficeWorldPatrolABEnv(OfficeWorldEnv):
    ...
    def _get_suggested_actions(self, agent_pos, target):
        """Returns a set of suggested actions (up, down, left, right) to guide the agent towards the target position."""
        
        for pos, elem in self.locations.items():
            if elem == target:
                target_pos = pos
    ...
    def guide_agent(self):
        if self.visited_rooms == OfficeWorldRoomVisits.VISITED_NONE:
