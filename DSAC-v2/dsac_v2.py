...
class ApproxContainer(torch.nn.Module):
    ...
    def create_action_distributions(self, logits, epsilon):
        return self.policy.get_act_dist(logits, epsilon)
...
class DSAC_V2:
    ...
    def local_update(self, data: Dict, iteration: int, epsilon: float) -> dict:
        tb_info = self.__compute_gradient(data, iteration, epsilon)
    ...
    def __compute_gradient(self, data: Dict, iteration: int, epsilon: float) -> dict:
        ...
        act_dist = self.networks.create_action_distributions(logits, epsilon=epsilon)
    ...
    def __compute_loss_q(self, data: Dict):
        ...
        act2_dist = self.networks.create_action_distributions(logits_2, epsilon=0.0)
